...
class LocalFileStore:
    ...
    def delete(self, key: str) -> None:
        ...
        Args:
            key: Document ID
        """
        validated_key = self._validate_doc_id(key)
        file_path = self.path / f"{validated_key}.json"
        if file_path.exists():
            file_path.unlink()
    ...
    def _save_document(self, doc_id: str, doc: Document) -> None:
        ...
        """
        validated_id = self._validate_doc_id(doc_id)
        file_path = self.path / f"{validated_id}.json"
        doc_data = {
    ...
    def _validate_doc_id(self, doc_id: str) -> str:
        ...
        Raises:
            ValueError: If document ID is invalid or would escape store directory
        """
        # Sanitize the doc_id to remove dangerous characters
